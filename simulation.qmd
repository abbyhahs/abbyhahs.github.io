...
histogram(dataset = null_hypothesis2, xvar = chisq2, value = chisq_s, title = "Simulated Chi-Squared Test Statistic", x = "Chi-Squared Values")

p_value2 <- sum(abs(chisq2) >= abs(chisq_s)) / 1000
p_value2
```

The plot depicts a histogram of the 1000 simulated chi-squared values for the data. The histogram is skewed right, which is expected for a chi-squared distribution. The red line marks the chi-squared statistic for the observed data which is 22.1. The difference between the simulated chi-squared values and the observed value, along with the p value < 0.05 tells us there is significant evidence that the different options of opt-in, opt-out, and neutral have an effect on whether or not individuals decide to be a donor.
